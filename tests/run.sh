...
echo "== unit: effort_label =="
assert_eq "low -> Low"        "Low"    "$(effort_label low)"
assert_eq "medium -> Medium"  "Medium" "$(effort_label medium)"
assert_eq "high -> High"      "High"   "$(effort_label high)"
assert_eq "xhigh -> XHigh"    "XHigh"  "$(effort_label xhigh)"
assert_eq "max -> Max"        "Max"    "$(effort_label max)"
assert_eq "unknown -> as-is"  "turbo"  "$(effort_label turbo)"

echo "== unit: effort_color =="
assert_eq "low -> green"      "${ESC}[32m"    "$(effort_color low)"
assert_eq "medium -> cyan"    "${ESC}[36m"    "$(effort_color medium)"
assert_eq "high -> yellow"    "${ESC}[33m"    "$(effort_color high)"
assert_eq "xhigh -> magenta"  "${ESC}[35m"    "$(effort_color xhigh)"
assert_eq "max -> red"        "${ESC}[01;31m" "$(effort_color max)"
assert_eq "unknown -> cyan"   "${ESC}[36m"    "$(effort_color whatever)"
...
echo "== integration: reasoning effort next to the model =="
OUT="$(render "$(cat "$FIX/stdin_effort.json")" "$FIX/config_max.json")"
assert_contains "model with effort" "$OUT" "Opus 4.8 High"
# Coloured apart from the model name (yellow for high), label reset after.
RAW="$(render_raw "$(cat "$FIX/stdin_effort.json")" "$FIX/config_max.json")"
assert_contains "effort label coloured" "$RAW" "${ESC}[33mHigh${ESC}[0m"

echo "== integration: no effort field -> plain model, no effort label =="
OUT="$(render "$(cat "$FIX/stdin_basic.json")" "$FIX/config_max.json")"
assert_contains     "plain model shown"   "$OUT" "Claude Opus 4.8"
assert_not_contains "no effort appended"  "$OUT" "Claude Opus 4.8 High"

echo "== integration: SHOW_EFFORT=0 hides effort, keeps model =="
RC_NOEFF="$(mktemp)"; printf 'SHOW_EFFORT=0\n' > "$RC_NOEFF"
OUT="$(render_rc "$(cat "$FIX/stdin_effort.json")" "$FIX/config_max.json" "$RC_NOEFF")"
assert_contains     "model still shown" "$OUT" "Opus 4.8"
assert_not_contains "effort hidden"     "$OUT" "High"
rm -f "$RC_NOEFF"
