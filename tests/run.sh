...
assert_contains "double dot in tag" "$OUT" "[Max \$●●]"
...
assert_contains "dot reflects worst (7d)" "$OUT" "[Max \$●]"

echo "== integration: extra usage OFF -> no \$ / no dot =="
OUT="$(render "$(cat "$FIX/stdin_basic.json")" "$FIX/config_extra_off.json")"
assert_contains "plain plan tag" "$OUT" "[Max]"
assert_not_contains "no dot"     "$OUT" "●"
assert_not_contains "no \$ marker" "$OUT" "\$ ]"
